 ...
 *   bidirectional serial communication over USART2 with
 *   interrupt-driven reception using a circular buffer and blocking
 *   transmission. It includes functions to send characters/strings,
 *   receive complete strings terminated by carriage return or newline,
 *   and initialize the UART hardware with 115200 baud rate.
 */
#include 	"uart.h"
...
void init_uart(void){
		...
		*((volatile uint32_t *)(RCC_BASE_ADDR + RCC_APB1ENR_OFFSET))	|= (0x1 << 17);
		// Uart on pin PA2 Tx
		*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOA_MODER_OFFSET))	&= ~(0x3 << 2*2); //Alternate function
		*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOA_MODER_OFFSET))	|= (0x2 << 2*2); //Alternate function
		*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOA_AFRL_OFFSET))	&= ~(0xF << 4*2); //AF 7 for Tx
		*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOA_AFRL_OFFSET))	|= (0x7 << 4*2); //AF 7 for Tx
		// Uart on pin PA3 Rx
		*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOA_MODER_OFFSET))	&= ~(0x3 << 2*3); //Alternate function
		*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOA_MODER_OFFSET)) |= (0x2 << 2*3); //Alternate Function mode (MODER bits [7:6])
		*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOA_AFRL_OFFSET))	&= ~(0xF << 4*3); //AF 7 for Tx
