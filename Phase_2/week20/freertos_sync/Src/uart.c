 ...
 *
 * Description:
 *   This file initializes and manages UART2 communication on an
 *   STM32F446RE microcontroller, configuring GPIO pins PA2/PA3 for
 *   serial I/O at 115200 baud, and implements an interrupt handler
 *   that queues received characters into a FreeRTOS message queue for
...
static uint32_t xQueue1OverflowCount=0;
...
void USART2_IRQHandler(void){
	if( *((volatile uint32_t*)(USART2_BASE_ADDR + USART_SR_OFFSET)) & (0x1 << 5) ){ // Check if interrupt fired because data has been received
		...
		if (xQueueSendFromISR(xQueue1, &item, &xHigherPriorityTaskWoken) != pdTRUE) {
		    xQueue1OverflowCount++;  // volatile global, inspect in debugger or log task
		}
