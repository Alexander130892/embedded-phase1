 ...
 *
 * Description:
 *   This file implements FreeRTOS stream buffer functionality,
 *   providing thread-safe inter-task communication primitives that use
 *   task notifications to coordinate sending and receiving of data
 *   streams. The implementation is conditionally compiled based on
 *   FreeRTOS configuration flags and supports optional per-instance
