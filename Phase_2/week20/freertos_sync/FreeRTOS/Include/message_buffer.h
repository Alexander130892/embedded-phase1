 ...
 *
 * Description:
 *   This file defines the interface for FreeRTOS message buffers,
 *   which are built on top of stream buffers to enable variable-length
 *   discrete message passing between tasks or interrupts. Message
 *   buffers store each message with an additional size_t header to
 *   track message length, and require single-writer/single-reader
 *   semantics to operate safely without synchronization.
 */
/*
