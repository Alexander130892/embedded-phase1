 ...
 *
 * Description:
 *   This file defines macros for detecting task stack overflow in
 *   FreeRTOS by comparing the current stack pointer against configured
 *   limits, with support for both downward and upward stack growth
 *   architectures. It provides configurable overflow detection levels,
 *   with stricter checks available when
 *   `configCHECK_FOR_STACK_OVERFLOW` is set to values greater than 1,
 *   and skips checks for MPU-protected ports that have hardware
 *   overflow detection.
 */
