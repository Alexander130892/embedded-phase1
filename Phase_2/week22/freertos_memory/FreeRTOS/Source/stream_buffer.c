 ...
 *   providing inter-task communication through a byte stream
 *   abstraction with task notification support. It is conditionally
 *   compiled only when `configUSE_STREAM_BUFFERS` is enabled and
 *   requires task notifications and task handle access features to be
 *   configured.
 */
/* Standard includes. */
#include <string.h>
