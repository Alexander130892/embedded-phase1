 ...
 *
 * Description:
 *   This header file defines prototypes for MPU (Memory Protection
 *   Unit) wrapper versions of FreeRTOS kernel API functions, which
 *   intercept standard API calls to enforce privilege level
 *   transitions and memory protection policies. It includes type
 *   definitions for parameter structs used to pass arguments across
 *   privilege boundaries and declares MPU-prefixed function prototypes
 *   that wrap the original kernel functions with privilege promotion
 *   and demotion logic.
 */
/*
