 ...
 *
 * Description:
 *   This header file defines the API and types for FreeRTOS stream
 *   buffers, which are lightweight communication primitives for
 *   passing continuous data streams between tasks or interrupts. A
 *   critical constraint is that stream buffers assume only one writer
 *   and one reader; multiple writers or readers must be serialized by
 *   the application.
 */
/*
