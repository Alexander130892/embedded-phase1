 ...
 *
 * Description:
 *   This file provides Memory Protection Unit (MPU) wrapper macros
 *   that redirect standard FreeRTOS kernel API function calls to
 *   MPU-protected equivalents, enabling privilege escalation/demotion
 *   so that kernel code runs with full privileges while application
 *   code runs unprivileged. It conditionally applies these macro
 *   redirections only when MPU support is enabled and the file is
 *   included from application code (not from internal kernel files).
 */
