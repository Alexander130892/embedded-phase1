 ...
 *
 * Description:
 *   This header file defines the FreeRTOS stream buffer API, a
 *   lightweight inter-task and interrupt communication mechanism that
 *   allows continuous data streaming between a single writer and
 *   single reader. It provides type definitions, function
 *   declarations, and callback mechanisms for managing stream buffers,
 *   message buffers, and batching buffers, with the critical
 *   constraint that multiple writers or readers must be serialized by
 *   the application.
 */
/*
