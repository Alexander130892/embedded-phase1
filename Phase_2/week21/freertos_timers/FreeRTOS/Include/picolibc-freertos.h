 ...
 *
 * Description:
 *   This header file integrates picolibc's thread-local storage (TLS)
 *   support with FreeRTOS by defining macros that allocate,
 *   initialize, and manage TLS blocks for each FreeRTOS thread. It
 *   handles TLS memory layout on the thread stack with proper
 *   alignment, accounting for different stack growth directions and
 *   picolibc versions.
 */
