 ...
 *
 * Description:
 *   This file provides MPU (Memory Protection Unit) security wrappers
 *   by remapping standard FreeRTOS kernel API functions to protected
 *   equivalents that enforce privilege level transitions, ensuring
 *   kernel code runs with full privileges while application code runs
 *   with restricted privileges. It only applies these mappings when
 *   MPU protection is enabled and the file is included from
 *   application code (not internal kernel files).
 */
