 ...
 *
 * Description:
 *   This header file defines the API for FreeRTOS stream buffers,
 *   lightweight inter-task and interrupt communication objects that
 *   efficiently transfer continuous streams of data between a single
 *   writer and single reader. The implementation emphasizes that
 *   multiple writers or readers must be externally serialized, and
 *   provides opaque handle-based access to stream buffer operations
 *   with configurable trigger levels for unblocking waiting tasks.
 */
/*
