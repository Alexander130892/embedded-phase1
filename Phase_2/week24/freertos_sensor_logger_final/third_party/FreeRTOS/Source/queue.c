 ...
 *
 * Description:
 *   This file implements FreeRTOS queue management, providing core
 *   data structures and functionality for inter-task communication via
 *   queues, mutexes, and semaphores. It defines the queue internal
 *   representation and handles the locking, blocking, and task
 *   synchronization mechanisms required for safe queue operations in a
