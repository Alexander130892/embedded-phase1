...
status_t uart2_init(void){
	...
	*((volatile uint32_t *)(RCC_BASE_ADDR + RCC_APB1ENR_OFFSET))	|=  (1u << 17);		//Enable clock for USART
	...
	// USART2 Config
	*((volatile uint32_t *)(USART2_BASE_ADDR + USART_BRR_OFFSET))	= UART_BAUDRATE_9600;
...
status_t uart2_gpio_init(void){
	...
	*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOx_AFRL_OFFSET))	|= (UART2_ALT_FUNC << 4*2); //AF 7 for Tx
	// Uart on pin PA3 Rx
	...
	*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOx_AFRL_OFFSET))  |= (UART2_ALT_FUNC << 4*3); // AF7 (AFRL bits [15:12])
	return STATUS_OK;
}
status_t uart4_init(void){
	// Enable clocks
	*((volatile uint32_t *)(RCC_BASE_ADDR + RCC_APB1ENR_OFFSET))	|=  (1u << 19);		//Enable clock for USART

	// USART4 Config
	*((volatile uint32_t *)(USART4_BASE_ADDR + USART_BRR_OFFSET))	= (UART_BAUDRATE_9600);
	*((volatile uint32_t *)(USART4_BASE_ADDR + USART_CR1_OFFSET))	|= (0x1 << 3); //UART Enable TE - bit 3
	*((volatile uint32_t *)(USART4_BASE_ADDR + USART_CR1_OFFSET)) 	|= (0x1 << 2); //UART Enable RE - bit 2
	//	Enable Interrupt RXNEIE
	*((volatile uint32_t*)(USART4_BASE_ADDR + USART_CR1_OFFSET)) 	|= 	(0x1 << 5);
	// 	Enable NVIC
	*((volatile uint32_t*)(NVIC_BASE_ADDR + NVIC_ISER1_OFFSET))		|= (0x1 << UART4_POS);
	*((volatile uint32_t *)(USART4_BASE_ADDR + USART_CR1_OFFSET))	|= (0x1 << 13); //UART Enable UE - bit 13
	return STATUS_OK;
}
status_t uart4_gpio_init(void){
	...
	*((volatile uint32_t *)(RCC_BASE_ADDR + RCC_AHB1ENR_OFFSET)) 	|= 	(1u << 0);		//  Bit 0 for GPIOA
	// Uart on pin PA0 Tx
	*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOx_MODER_OFFSET))	&= ~(0x3 << 2*0); //Alternate function
	*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOx_MODER_OFFSET))	|= (0x2 << 2*0); //Alternate function
	*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOx_AFRL_OFFSET))	&= ~(0xF << 4*0); //AF 8 for Tx
	*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOx_AFRL_OFFSET))	|= (UART4_ALT_FUNC << 4*0); //AF 8 for Tx
	// Uart on pin PA1 Rx
	*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOx_MODER_OFFSET))	&= ~(0x3 << 2*1); //Alternate function
	*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOx_MODER_OFFSET)) |= (0x2 << 2*1); //Alternate Function mode (MODER bits [7:6])
	*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOx_AFRL_OFFSET))	&= ~(0xF << 4*1); //AF 8 for Tx
	*((volatile uint32_t *)(GPIOA_BASE_ADDR + GPIOx_AFRL_OFFSET))  |= (UART4_ALT_FUNC << 4*1); // AF8 (AFRL bits [15:12])
	return STATUS_OK;
}
...
status_t uart4_transmit(uint8_t c){
	volatile uint32_t timeout = 0;
	while(!(*((volatile uint32_t *)(USART4_BASE_ADDR + USART_SR_OFFSET)) & (0x1 << 7))){// Wait until TXE is 1 (Tx data register empty)
		timeout++;
		if(timeout > MAX_TIMEOUT) return UART_TIMEOUT;
	}
	*((volatile uint32_t *)(USART4_BASE_ADDR + USART_DR_OFFSET)) = c;
	return STATUS_OK;
}
...
void UART4_IRQHandler(void){
	if( *((volatile uint32_t*)(USART4_BASE_ADDR + USART_SR_OFFSET)) & (0x1 << 5) ){ // Check if interrupt fired because data has been received
		buffer[head]= *((volatile uint32_t *)(USART4_BASE_ADDR + USART_DR_OFFSET));
