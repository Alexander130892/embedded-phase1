...
void door_fsm_init(void) {
    ...
    ledOn(LED_B_PIN);   // set initial LED
}
    ...
    switch(door_state){
        case DOOR_CLOSED:   // BLUE LED
            if(ev.type == EV_BUTTON_START){
                ...
                // Entry
                postEvent((Event){EV_DOOR_UNLOCKED, 0});   // notify other FSMs
                ledOn(LED_G_PIN);   /* Toggle GREEN LED */
        ...
        case DOOR_OPENING:  
            if(ev.type == EV_LIMIT_OPEN){
                //Exit
                ledOff(LED_G_PIN);   /* Toggle GREEN LED */
                // State change
                door_state = DOOR_OPEN;
                // Entry
                postEvent((Event){EV_DOOR_OPEN, 0});   // notify other FSMs
                ledOn(LED_R_PIN);   /* Toggle RED LED */
        ...
        case DOOR_OPEN:   
            if(ev.type == EV_BUTTON_START || ev.type == EV_TIMEOUT){
                //Exit
                ledOff(LED_R_PIN);   /* Toggle RED LED */
                // State change
        ...
        case DOOR_CLOSING:
            if(ev.type == EV_LIMIT_CLOSED){
                ...
                ledOn(LED_B_PIN);   /* Toggle BLUE LED */
